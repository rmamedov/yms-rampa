...
test.describe('S-08 Маршрутні листи', () => {
  ...
  test('S-08.4 призначення водія на весь лист', async ({ page }) => {
    ...
    // Обмеження знято: бекенд приймає порожній driverId для листа цілком, тож
    // «Водія не призначено» — звичайний вибір, а не недоступна позначка.
    // Підказка мусить пояснювати, що саме він робить (перевірку самої дії
    // робить S-08.10).
    await expect(
      page.locator('.sheet-driver'),
      'кабінет має пояснювати, що робить вибір «Водія не призначено»',
    ).toContainText('знімає водія з усього листа');
    expect(
      await page.locator('#sheet-driver option[value=""]').getAttribute('disabled'),
      'варіант «Водія не призначено» має бути доступним для вибору',
    ).toBeNull();
  });
