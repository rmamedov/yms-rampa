...
/**
 * За скільки хвилин до слоту відкривається відмітка «На місці» (розділ 8).
 *
 * ДЗЕРКАЛО `StorePolicy::ARRIVAL_WINDOW_MINUTES` у booking-service — там
 * єдине джерело істини. Значення продубльоване тут лише тому, що бекенд не
 * віддає його в маршрутному листі; якщо домен змінить ширину вікна, цей
 * рядок треба привести у відповідність, інакше кнопка зʼявлятиметься раніше,
 * ніж сервер готовий прийняти відмітку.
 */
export const ARRIVAL_WINDOW_MINUTES = 60;

/** Момент, коли відмітка «На місці» на цій точці стає доступною, epoch ms. */
export function arrivalOpensAt(point: RoutePoint): number {
  return Date.parse(point.slotStart) - ARRIVAL_WINDOW_MINUTES * 60_000;
}

/**
 * Чи можна вже відмітити прибуття на цю точку (розділ 8, D-04).
 *
 * ДЗЕРКАЛО ДОМЕНУ. Джерело істини — ArrivalWindow у booking-service: раніше
 * ніж за ARRIVAL_WINDOW_MINUTES до слоту відмітка повертає 422
 * ARRIVAL_TOO_EARLY. Тут те саме правило повторене лише для того, щоб водій
 * не тиснув кнопку, яку сервер відхилить, — і щоб екран пояснював, коли вона
 * стане доступною. Верхньої межі немає: після кінця слоту прибуття
 * приймається з позначкою запізнення.
 */
export function arrivalAvailable(
  point: RoutePoint,
  now: number = Date.now(),
): boolean {
  return now >= arrivalOpensAt(point);
}
