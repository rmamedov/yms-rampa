...
{
    /**
     * DRV/ST-01, рівень мережі: за скільки хвилин ДО початку слоту
     * відкривається відмітка «На місці» (розділ 8, вікно «−60 хв … кінець
     * слоту»). ЄДИНЕ місце, де живе ширина вікна: домен бере її звідси
     * (ArrivalWindow), застосунок водія дзеркалить те саме значення.
     *
     * Поки що константа, а не поле конструктора: рівень магазину для цього
     * параметра store-service не віддає. Коли віддасть — константа стане
     * дефолтом поля, і решта коду не зміниться.
     */
    public const int ARRIVAL_WINDOW_MINUTES = 60;
