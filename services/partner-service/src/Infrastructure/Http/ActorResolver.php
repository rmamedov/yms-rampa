 ...
 *
 * Шлюз виставляє всі пʼять заголовків ПРИМУСОВО (fastcgi_param перекриває
 * однойменний заголовок клієнта), тож підробити їх ззовні не можна. Сервіс
 * токен не перевіряє — лише читає результат перевірки.
 */
final readonly class ActorResolver
{
    ...
    public function fromRequest(Request $request): Actor
    {
        ...
        return new Actor(
            ...
            // Порожній рядок = «не застосовно»; для ролі постачальника це
            // відмова, і кидає її вже конструктор актора.
    ...
    /**
     * Постачальник, у межах якого працює кабінет партнера, з перевіркою права
     * за матрицею 4.4. Контролери беруть ідентифікатор тільки звідси.
     *
     * @throws AccessDeniedException
     */
    public function ownSupplierId(Request $request, Permission $permission): string
    {
        return $this->fromRequest($request)->requireOwnSupplierScope($permission);
    }

    /**
     * Мережевий доступ до адмін-API (staff-контур).
     *
     * @throws AccessDeniedException
     */
    public function requireNetworkWide(Request $request, Permission $permission): Actor
    {
        $actor = $this->fromRequest($request);
        $actor->requireNetworkWide($permission);
     ...
     * Відсутній заголовок — контур виводиться з ролі (сумісність зі старішими
